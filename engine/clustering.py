...
def preprocess_features(npdata, norm=False, pca=256):
...
def cluster_assign(pixels_lists, dataset):
    ...
    Returns:
        ReassignedDataset(torch.utils.data.Dataset): a dataset with clusters as
                                                     labels
    """
    assert pixels_lists is not None
    pseudolabels = []
    image_indexes = []
    for cluster, images in enumerate(pixels_lists):
...
def run_kmeans(x, nmb_clusters, device, min_num=100, max_num=20000, verbose=False):
...
def arrange_clustering(pixels_lists):
    pseudolabels = []
    image_indexes = []
    for cluster, images in enumerate(pixels_lists):
        image_indexes.extend(images)
...
class Kmeans:
    ...
    def cluster(self, data, device=0, min_num=100, max_num=20000, verbose=False):
        ...
        xb = preprocess_features(data, norm=self.norm)

        # cluster the data
        I, loss = run_kmeans(xb, self.k, device, min_num, max_num, verbose)
        self.pixels_lists = [[] for i in range(self.k)]
        for i in range(len(data)):
            self.pixels_lists[I[i]].append(i)
...
class PIC():
    ...
    def cluster(self, data, verbose=False):
        ...
        for data, c in enumerate(clust):
            pixels_lists[c].append(data)

        # allocate singletons to clusters of their closest NN not singleton
        if self.distribute_singletons:
            clust_NN = {}
            for i in pixels_lists:
                # if singleton
                if len(pixels_lists[i]) == 1:
                    s = pixels_lists[i][0]
                    # for NN
                    for n in I[s, 1:]:
                        # if NN is not a singleton
                        if not len(pixels_lists[clust[n]]) == 1:
                            clust_NN[s] = n
                            break
            for s in clust_NN:
                del pixels_lists[clust[s]]
                clust[s] = clust[clust_NN[s]]
                pixels_lists[clust[s]].append(s)

        self.pixels_lists = []
        for c in pixels_lists:
            self.pixels_lists.append(pixels_lists[c])
